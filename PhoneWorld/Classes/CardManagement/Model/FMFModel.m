...
- (instancetype)initWithFMVideoViewType:(FMVideoViewType)type superView:(UIView *)superView andTypeString:(NSString *)typeString
{
    self = [super init];
    if (self) {
...
- (void)setUpVideo
{
    ...
    if ([self.typeString isEqualToString:@"工号实名制"] || [self.typeString isEqualToString:@"靓号"]) {
        //默认前置
    }else{
        NSDictionary *modeDic = [[NSUserDefaults standardUserDefaults] objectForKey:@"kaihuMode"];
        NSNumber *positionNum = modeDic[@"shootModes"];
        if (positionNum) {
            if (positionNum.intValue == 2) {//后置
...
- (void)setUpPreviewLayerWithType:(FMVideoViewType )type
{
    ...
    switch (type) {
        ...
        case TypeFullScreen:
            ...
            break;
        default:
            rect = CGRectMake(40*getScale, 70*getScale, SCREEN_WIDTH-(40*getScale*2), SCREEN_WIDTH-(40*getScale*2));
            break;
    }
    
    self.previewlayer.frame = rect;
    self.previewlayer.cornerRadius = (SCREEN_WIDTH-(40*getScale*2))/2;
...
- (void)backToPreviousPageToStopRecord{
    [self.session stopRunning];
    if (self.timer) {
        [self.timer invalidate];
        self.timer = nil;
