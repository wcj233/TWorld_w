...
- (void)createCameraView{
    ...
    if (kScreenHeight > SCREEN_WIDTH) {
        aWidth = SCREEN_WIDTH;
        aHeight = kScreenHeight;
    }else{
        aWidth = kScreenHeight;
        aHeight = SCREEN_WIDTH;
    }
...
//    NSLog(@"roi%d", a);
    ...
    _lightspotSwitch.frame = CGRectMake(SCREEN_WIDTH/2-50, kScreenHeight-65, 100, 30);
    ...
    _lightspotLabel.center = CGPointMake(self.view.center.x-100, self.view.center.y);
...
//                UIImageWriteToSavedPhotosAlbum(image, nil, nil, nil);
                ...
                if (hidden || sliderLine.allLine == 1033 || sliderLine.allLine == 1034 || sliderLine.allLine == 1036)
                {
...
CGRectMakeBack(CGFloat x, CGFloat y, CGFloat width, CGFloat height)
{
    ...
    if (kScreenHeight==480) {
        ...
        rect.origin.x = x * SCREEN_WIDTH/320;
        rect.origin.y = y * kScreenHeight/568;
        rect.size.width = width * SCREEN_WIDTH/320;
