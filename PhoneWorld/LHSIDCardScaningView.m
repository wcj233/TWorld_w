...
-(void)addScaningWindow {
    ...
    if (kDevice_Is_iPhoneX || iPhone6Plusor6sPlusor7Plus) {
        _IDCardScanningWindowLayer.bounds = (CGRect){CGPointZero, {width, width * 1.574}};
    }else {
        _IDCardScanningWindowLayer.bounds = (CGRect){CGPointZero, {width, SCREEN_HEIGHT-(kStatusBarHeight+kButton_bottom+49*2+20+30)}};
    }
    
    CGRect frame  = _IDCardScanningWindowLayer.frame;
//    frame.origin.y = kStatusBarHeight+49+15;;
    frame.origin.y = (SCREEN_HEIGHT-_IDCardScanningWindowLayer.bounds.size.height)/2;
    _IDCardScanningWindowLayer.frame = frame;
...
//    CGFloat facePathWidth = iPhone5or5cor5sorSE? 100: (iPhone6or6sor7? 120: 130);
//    CGFloat facePathHeight = facePathWidth * 0.82;
...
//    self.facePathRect = (CGRect){CGRectGetMaxX(rect) - facePathWidth - 35,CGRectGetMaxY(rect) - facePathHeight - 25,facePathWidth,facePathHeight};
    ...
    CGRect rect = _IDCardScanningWindowLayer.frame;
    self.facePathRect = (CGRect){CGRectGetMaxX(rect) - facePathWidth - 37,CGRectGetMaxY(rect) - facePathHeight - 25,facePathWidth,facePathHeight};
...
//    center.x = CGRectGetMaxX(_IDCardScanningWindowLayer.frame) + 20;
    ...
    if ([self.type isEqualToString:@"身份证正面照"]) {
        [self addTipLabelWithText:@"请拍摄身份证人像面，并尝试对齐边缘" center:center];
    }else {
        [self addTipLabelWithText:@"请拍摄身份证国徽面，并尝试对齐边缘" center:center];
    }
...
//    frame.size.height = _IDCardScanningWindowLayer.frame.size.height;
